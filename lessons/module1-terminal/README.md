...
<div align="center">
  <img src="https://raw.githubusercontent.com/OlegKarenkikh/devops-for-kids/main/images/module1-filesystem.jpg" alt="Файловая система Linux" width="900"/>
  <br/><em>🌳 Файловая система Linux — это дерево: корень / наверху, ветки-папки расходятся вниз до твоей домашней папки</em>
</div>
...
<div align="center">
  <img src="https://raw.githubusercontent.com/OlegKarenkikh/devops-for-kids/main/images/module1-bash-script.jpg" alt="Структура Bash-скрипта" width="900"/>
  <br/><em>👨‍🍳 Bash-скрипт — это рецепт для робота-повара: список шагов, которые он выполняет строго по порядку</em>
</div>
...
## ❓ Вопросы новичков — Терминал

<details>
<summary><strong>Почему команды такие короткие (ls, cd, pwd)?</strong></summary>

Их придумали в **1970-х годах**, когда клавиатуры были медленными, а интернет передавал данные побайтово. Чем короче команда — тем быстрее набрать.
...
Со временем запомнишь автоматически — как буквы на клавиатуре.

</details>

<details>
<summary><strong>Знак $ в начале строки — это часть команды?</strong></summary>
...
<details>
<summary><strong>Что такое root и зачем он нужен?</strong></summary>

Root — **главный администратор** системы Linux. Он может всё: удалить любой файл, сломать систему, установить любую программу.

```bash
whoami        # Узнать кто ты сейчас
sudo whoami   # Выполнить как root (покажет: root)
```

> ⚠️ Правило: не работай под root постоянно. Одна опечатка — можно удалить всю систему. Используй `sudo` только когда действительно нужно.

</details>

<details>
<summary><strong>Зачем нужен bash-скрипт?</strong></summary>

Скрипт — это **список команд** в файле, чтобы выполнить их все сразу.

```bash
# Без скрипта — каждый раз вручную (5 команд):
git pull && npm install && npm run build && docker build -t myapp . && docker push myapp

# Со скриптом — одна команда:
./deploy.sh
```

Скрипт — как макрос в Excel: один раз написал, потом запускаешь одной командой.

</details>

<details>
<summary><strong>Что такое chmod 755?</strong></summary>

<div align="center">
<img src="https://raw.githubusercontent.com/OlegKarenkikh/devops-for-kids/main/images/kid_chmod.jpg" alt="chmod 755 расшифровка" width="85%"/>
<br/><em>Три цифры = три группы. Каждая цифра = сумма прав: 4=читать, 2=писать, 1=запускать</em>
</div>

**Три группы людей:** владелец → группа → все остальные  
**Три вида прав:** r=4 (читать), w=2 (писать), x=1 (запускать)

- `7` = 4+2+1 = всё
- `5` = 4+0+1 = читать + запускать
- `4` = 4+0+0 = только читать

```bash
chmod 755 script.sh   # Владелец: всё. Остальные: читать+запускать
