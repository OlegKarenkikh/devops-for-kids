...
| Вопрос | Ответ |
|--------|-------|
| Чем образ (image) отличается от контейнера? | Образ — шаблон (статичный), контейнер — запущенный процесс |
| Что такое слой (layer) в Docker? | Каждая инструкция Dockerfile создаёт слой (кэшируется) |
| Зачем `COPY . .` после `pip install`? | Кэш: зависимости не пересобираются если код изменился |
| Что делает `docker run -p 8080:80`? | Проброс порта: хост:8080 → контейнер:80 |
| Зачем `docker system prune`? | Удаляет остановленные контейнеры, неиспользуемые образы |
| Что такое `.dockerignore`? | Список файлов, которые НЕ копируются в образ (как .gitignore) |
